...
sortedByBirthYear = inventors.sort((inventorA, inventorB) => {
...
inventorNamedAda = inventors.find(({first}) => {
    return first === 'Ada'
})
...
//     firstLast[i] = (people[i].split(",").sort().join(' '))
// }
...
isAdultPresent = devs.some((adult) => {
...
idx = comments.findIndex((IdIdx) => {
    return (IdIdx.id === 123523)
})
...
totalYearsLived = inventors.reduce((lifeSpan, inventor) => lifeSpan + (inventor.passed - inventor.year), 0);
